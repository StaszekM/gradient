...
class SupervisedNodeClassificationGNN(pl.LightningModule):
    ...
    def validation_step(self, batch: List[Data], batch_idx: int) -> dict:
        ...
        y_pred, y, auc, f1, accuracy, precision, recall= self._common_step(data=data, mask=data.val_mask)
    ...
    def test_step(self, batch: List[Data], batch_idx: int):
        ...
        y_pred, y, auc, f1, accuracy, precision, recall  = self._common_step(data=data, mask=data.test_mask)
        ...
        return {"auc": auc, 'f1': f1, 'y_pred': y_pred, 'y': y, 'asccuracy': accuracy, 'precision': precision, 'recall': recall}
    ...
    def _common_step(
        ...
        auc = roc_auc_score(
            ...
            y_score=y_pred_b.detach().cpu().numpy(),
            multi_class="ovr",
            average='weighted', 
        )
