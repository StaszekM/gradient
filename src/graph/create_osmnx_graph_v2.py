    ...
    def get_edge_attrs(self):
        ...
        * other columns - add default value of "unspecified"
        ...
        if not any(col in attrs.columns for col in ['highway','osmid', 'access', 'junction', 'bridge', 'tunnel', 'geometry']):
            ...
            attrs['maxspeed'] = attrs['maxspeed'].astype(str).apply(check_max_speed)
            attrs['maxspeed'] = attrs["maxspeed"].replace('nan', np.nan)
            attrs['maxspeed'] = attrs["maxspeed"].fillna(attrs['maxspeed'].astype(float).mean())
            ...
            for col in ['highway', 'access', 'junction', 'bridge', 'tunnel']:
                columns_to_add = [item for item in all_attributes[f"edges_{col}"] if item not in set(attrs[col])]
                ...
                new_index_tuples = [(u, v, x, n) for u, v, x, n in attrs.index]
                new_index = pd.MultiIndex.from_tuples(new_index_tuples, names=['u', 'v', 'key', None])
            ...
            for col in ['highway_0', 'access_0', 'junction_0', 'bridge_0', 'tunnel_0']:
