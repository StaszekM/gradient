...
def get_ids_list_for_file(filename):
    with open(filename) as f:
        ...
        accidents_object = loaded_json["mapa"]["wojewodztwa"][0]["powiaty"][0]["gminy"][
            0
        ]

        metadata: Dict[str, Any] = {
            key: (
                accidents_object[key].strip()
                if isinstance(accidents_object[key], str)
                else accidents_object[key]
            )
            for key in accidents_object
            if key != "zdarzenia_detale"
        }

        ids: List[int] = list(
            map(lambda detail: detail["id"], accidents_object["zdarzenia_detale"])
        )

    assert (
        len(ids) == metadata["zdarzenia"]
    ), f"Corrupted file: expected {metadata['zdarzenia']} accidents, got {len(ids)}"
    return ids, metadata


def scrape_file(filename, trials=10):
        ...
        for id in tqdm_bar:
            ...
            for i in range(trials):
                if downloaded:
                    break
                try:
                    tqdm_bar.set_description(f"{str(id)} (trial {i + 1}/{trials})")
                    r = requests.post(
                        SINGLE_EVENT_URL,
                        data=create_form_data_for_request(id),
                        timeout=10.0,
                    )
    ...
    file_name = (
        datetime.now().strftime("%Y_%M_%d_%H_%M_%S") + "_" + os.path.basename(filename)
    )
